...
plot_it <- F
  ...
  for(rep in 1:n_rep){
    x <- c(rnorm(n_data*w, -2, sd), rnorm(n_data*w, 2, sd))
    ...
    cu <- curve(w*pnorm(x,-2,sd)+w*(pnorm(x,2, sd)), lower_bound, upper_bound, n = length(res[[1]][[1]]), add = T, col = "red")
...
mean(sapply(res_list[1:74], function(x) (x[[1]])), na.rm = T)
mean(sapply(res_list[1:74], function(x) (x[[2]])))
mean(sapply(res_list[1:74], function(x) (x[[3]])))
